...
const convertToStrapiResume = (resume: Resume, userId?: number): Partial<StrapiResume> => {
  const { id, templateId, name, title, email, phone, location, website, summary, themeColor, profileImage, ...rest } = resume;
...
const convertToFrontendResume = (strapiResume: any): Resume => {
  ...
  const { name, title, email, phone, location, website, summary, themeColor, profileImage } = basicInfo || {};
...
export const strapiService = {
  ...
  async getUserByClerkId(clerkUserId: string): Promise<StrapiUser | null> {
    try {
      const response = await axios.get(`${API_URL}/api/users?filters[clerkUserId][$eq]=${clerkUserId}&populate=*`);
  ...
  async createUser(userData: { clerkUserId: string; name: string; email: string }): Promise<StrapiUser | null> {
    try {
      const response = await axios.post(`${API_URL}/api/users`, { data: userData });
  ...
  async getResumesByUserId(userId: number): Promise<Resume[]> {
    try {
      const response = await axios.get(`${API_URL}/api/resumes?filters[user][id][$eq]=${userId}&populate=*`);
  ...
  async createResume(resume: Resume, userId: number): Promise<Resume | null> {
    try {
      ...
      const response = await axios.post(`${API_URL}/api/resumes`, { data: strapiData });
  ...
  async updateResume(resume: Resume, strapiResumeId: number): Promise<Resume | null> {
    try {
      const strapiData = convertToStrapiResume(resume);
      const response = await axios.put(`${API_URL}/api/resumes/${strapiResumeId}`, { data: strapiData });
      return convertToFrontendResume(response.data.data);
    } catch (error) {
  ...
  mock: {
    ...
    async createUser(userData: { clerkUserId: string; name: string; email: string }): Promise<StrapiUser> {
    ...
    async getResumesByUserId(userId: number): Promise<Resume[]> {
      ...
      return [
        ...
        },
        {
          ...
          website: 'example.com',
          summary: 'Product manager with a track record of shipping successful products.',
          themeColor: '#10B981',
          experiences: [],
          education: [],
          skills: [],
